...
import { X, ExternalLink, Trash2, CheckCircle, XCircle, Loader, FileText, Download } from 'lucide-react';
...
export default function CampaignResultsModal({ campaign, onClose, onRepoDeleted }) {
  ...
  const handleDownloadCSV = () => {
    try {
      const successfulRepos = (campaign.results || []).filter(r => r.status === 'success');
      
      if (successfulRepos.length === 0) {
        alert('No successful repositories to export');
        return;
      }

      // Create CSV content
      const headers = ['Repository Name', 'Repository URL', 'Description', 'Account', 'Code Generated', 'Files Pushed'];
      const rows = successfulRepos.map(result => [
        result.repoName || '',
        result.repoUrl || '',
        (result.repoDescription || '').replace(/"/g, '""'), // Escape quotes
        result.account || '',
        result.codeGenerated ? 'Yes' : 'No',
      ...
      link.setAttribute('download', `${campaign.name.replace(/[^a-z0-9]/gi, '_')}_results.csv`);
  ...
  const readmeOnlyRepos = successfulRepos.filter(r => r.codeGenerated === false || r.codeGenerated === undefined);
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          {isSuccess && result.codeGenerated === false && (
                        ...
                        {isSuccess ? (
                          <>
                            <button
                              ...
                              className="text-sm text-blue-400 hover:text-blue-300 hover:underline flex items-center gap-1 w-fit cursor-pointer mb-1"
                            >
                              {result.repoUrl}
                            ...
                            {/* Code generation status and details */}
                            {result.codeGenerated && result.filesPushed > 0 ? (
                              <p className="text-xs text-neutral-500 mt-1">
                                {result.filesPushed} code files pushed
                              </p>
                            ) : result.codeGenerated === false ? (
                              <p className="text-xs text-yellow-500 mt-1">
                                Repository created with README only (code generation failed)
                              </p>
                            ) : (
